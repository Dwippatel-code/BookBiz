...
namespace BookBiz.Validator
{
    class Validation
    {
        public static bool IsValidID(TextBox text)
        {
            ...
            {
                MessageBox.Show("Invalid ID, it must be a 5 digit number", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Information);
                text.Clear();
                text.Focus();
                return false;
            }
        ...
        {
            int tempID;
            if ((text.TextLength != 4) || !((Int32.TryParse(text.Text, out tempID))))   
            {
